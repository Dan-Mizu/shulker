...
class DiscordWebhooks {
	...
	public init() {
		...
		if (this.config.DEBUG) {
			this.webhookClient.on("rateLimit", (data) =>
				console.log(
					`[DEBUG] Webhook is being rate limited. Timeout: ${
						data.timeout / 1000
					}s, Limit: ${data.limit}`
				)
			);
	...
	private async getUUIDFromUsername(
		username: string
	): Promise<string | null> {
		...
		try {
			...
			if (this.config.DEBUG)
				console.log(
					`[DEBUG] Fetched UUID ${uuid} for username "${username}"`
				);
			return uuid;
		} catch (e) {
			console.log(
				`[ERROR] Could not fetch uuid for ${username}, falling back to Steve for the skin`
			);
			return null;
		}
	}
	...
	private async makeDiscordWebhook(username: string, message: string) {
		const defaultHead = this.getHeadUrl(
			this.config.DEFAULT_PLAYER_HEAD ||
		...
		if (username === this.config.SERVER_NAME) {
			// use avatar for the server
			if (this.config.SERVER_IMAGE)
				messsageOptions.avatarURL = this.config.SERVER_IMAGE;
		} else {
			// use avatar for player
			const uuid = await this.getUUIDFromUsername(username);
			messsageOptions.avatarURL = !!uuid
				? this.getHeadUrl(uuid)
				: defaultHead;
		}
	...
	public async sendMessage(username: string, message: string) {
		...
		try {
			...
			console.log(
				"[ERROR] Could not send Discord message through WebHook! Falling back to sending through bot."
			);
